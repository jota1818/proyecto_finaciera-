function validarFormularioPreJudicial() {
    ...
    let fecha_acto = document.forms["preJudicialForm"]["fecha_acto"].value;
    ...
    let fecha_clave = document.forms["preJudicialForm"]["fecha_clave"].value;
    ...
    // Validaciones
    if (!acto || !["Inicio caso pre-judicial", "Notificación", "Amortización", "Cambio Gestor", "Postergación", "Fin de caso", "Pasa a judicial"].includes(acto)) {
        alert("El acto debe ser una de las siguientes acciones: Inicio caso pre-judicial, Notificación, Amortización, Cambio Gestor, Postergación, Fin de caso, Pasa a judicial."); return false;
    }

    if (!descripcion || descripcion.length > 100) {
        alert("La descripción debe tener un máximo de 100 palabras."); return false;
    }

    if (!notif_compromiso_pago_Evidencia || ![".docx", ".pdf", ".jpg", ".png"].some(ext => notif_compromiso_pago_Evidencia.endsWith(ext))) {
        alert("Debe adjuntar un archivo de notificación/compromiso de pago en formato docx, pdf, jpg o png."); return false;
    }

    if (fecha_clave && (!fecha_acto || new Date(fecha_clave) < new Date(fecha_acto))) {
        alert("La fecha clave no puede ser anterior a la fecha del acto."); return false;
    }

    if (dias_desde_fecha_clave && isNaN(dias_desde_fecha_clave)) {
        alert("Días desde fecha clave debe ser un número."); return false;
    }

    if (dias_de_mora && isNaN(dias_de_mora)) {
        alert("Días de mora debe ser un número."); return false;
    }

    if (dias_mora_PJ && isNaN(dias_mora_PJ)) {
        alert("Días de mora PJ debe ser un número."); return false;
    }

    if (interes && isNaN(interes)) {
        alert("El interés debe ser un número."); return false;
    }

    if (saldo_int && isNaN(saldo_int)) {
        alert("El saldo más interés debe ser un número."); return false;
    }

    if (monto_amortizado && isNaN(monto_amortizado)) {
        alert("El monto amortizado debe ser un número."); return false;
    }

    if (saldo_fecha && isNaN(saldo_fecha)) {
        alert("El saldo a la fecha debe ser un número."); return false;
    }

    // Validación de evidencias
    if (!evidencia1_localizacion || ![".jpg", ".png"].some(ext => evidencia1_localizacion.endsWith(ext))) {
        alert("Debe adjuntar una imagen para la evidencia 1 en formato jpg o png."); return false;
    }

    if (!evidencia2_foto_fecha || ![".jpg", ".png"].some(ext => evidencia2_foto_fecha.endsWith(ext))) {
        alert("Debe adjuntar una imagen para la evidencia 2 en formato jpg o png."); return false;
    }

    // Validación de objetivo logrado
    if (objetivo_logrado && !["SI", "NO"].includes(objetivo_logrado)) {
        alert("El objetivo logrado debe ser 'SI' o 'NO'."); return false;
    }

    // Validación de días desde fecha clave
    if (objetivo_logrado === "NO" && !dias_desde_fecha_clave) {
        alert("Si el objetivo no se logró, debe ingresar los días desde la fecha clave."); return false;
    }

    // Validación de actor
    if (actor && !["Gestor", "Cliente", "Supervisor", "Administrador"].includes(actor)) {
        alert("El actor debe ser uno de los siguientes: Gestor, Cliente, Supervisor, Administrador."); return false;
    }
    ...
    // Validación de monto amortizado
    if (monto_amortizado && isNaN(monto_amortizado)) {
        alert("El monto amortizado debe ser un número."); return false;
    }
